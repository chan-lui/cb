...
const config = {
    ...
    providers: [Credentials({
        ...
        authorize: async (credentials) => {
            try {
                const response = await fetch(`https://crestbank.vercel.app/api/verify`, {
                    method: 'POST',
                    headers: {
    ...
    callbacks: {
        ...
        session: async ({ session, token }) => {
            if (token) {
                ...
                const response = await axios.get(`https://crestbank.vercel.app/api/session-user?id=${token.sub}`);
